...
> **ℹ️ Deployed image.** The homelab `:latest` is built from `main` (tag
> `llm-gateway-v0.1.2`), which reconciles an earlier divergence: model-aware
> discovery/routing (ollama backends with `model: ""`) **and** the Groq `/openai/v1`
> health-check + chat fixes are all on `main` — so it's safe to rebuild `:latest`
> from `main`. (Tags `v0.1.0`/`v0.1.1` were a hotfix branch off `28c3603`, now folded
> in.) Consumers route via `llm-gateway.ai:4000` (e.g. linkedin-scout: Groq screening
> + `claude-sonnet-4-6` why_fit). The cfoperator agent intentionally does **not**
...
> All families above are emitted as of v0.1.2. `requests_total`, `tokens_total`,
> `request_duration_seconds`, and `backend_healthy` carry `app="llm-gateway"` (set by
> Prometheus' annotation pod-scrape), not a `container` label — query/dashboard on `app`.
