...
class WebServer:
    ...
    def _setup_routes(self):
            ...
            try:
                ...
                if tier == TIER_POD:
                    if not bridge.pod_tier:
                        reason = self._browser_pod_refusal(host, node, tier_note)
        ...
        def close_cockpit(investigation_id):
            ...
            try:
                ...
                # — which is also why the Job goes *whatever* tier was derived:
                # a terminal-side `attach --spawn` may have put a pod on the
                # same node under the CLI's rule while the drawer's session sits
                # on the host. The Job first, because it needs no ssh and so
                # cannot be stranded behind an unreachable host.
                removed = self._cockpit_spawner().destroy(investigation_id)
                if tier != TIER_POD:
                    removed = removed + self._cockpit_ladder().destroy(investigation_id, host=host)
            except CockpitSpawnError as e:
    ...
    def _browser_pod_refusal(self, host: str, node, tier_note: str) -> str:
        """Why the drawer cannot open this one, and what to do — the reason
        names the actual gap (no host, no inventory entry, a failed probe)
        rather than "in the cluster", which for a Pi that is both a node and a
        host was true and useless (CFOP-98)."""
